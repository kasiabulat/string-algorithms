        ...
        for i in range(1, 4):
            for j in range(1, 4):
                ...
                self.assertEqual(Q[i][j],
                                 [diff_between_columns[i * m][(j - 1) * m + 1], diff_between_columns[i * m][j * m]])
...
class TestRestoreLcsPart(unittest.TestCase):
    def test_restore_lcs_part(self):
        fr = four_russians_helpers(lcs_delete_cost_function, lcs_insert_cost_function, lcs_substitute_cost_function)
        reversed_lcs, i, j = fr.restore_lcs_part("#aa", "#ab", [0, 1, 1], [0, 1, 1], 2, 2, 2)

        self.assertEqual(reversed_lcs, "a")
        self.assertEqual(i, 0)
        self.assertEqual(j, 0)

        reversed_lcs, i, j = fr.restore_lcs_part("#aaccc", "#aabc#", [0, 1, 1, 1, 1, 1], [0, 1, 1, 1, 1, 1], 5, 5, 5)

        self.assertEqual(reversed_lcs[::-1], "aac")
        self.assertEqual(i, 0)
        self.assertEqual(j, 0)

        reversed_lcs, i, j = fr.restore_lcs_part("#aaa", "#bbb", [0, 1, 1, 1], [0, 1, 1, 1], 3, 3, 3)
